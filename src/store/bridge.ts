...
const getters = {
  isOutgoingDirection (state) {
    return state.isOutgoingDirection
  },
...
const mutations = {
  [types.SET_OUTGOING_DIRECTION] (state, isOutgoingDirection: boolean) {
    state.isOutgoingDirection = isOutgoingDirection
  },
...
const actions = {
  setOutgoingDirection ({ commit }, isOutgoingDirection: boolean) {
    commit(types.SET_OUTGOING_DIRECTION, isOutgoingDirection)
  },
  ...
  resetBridgeForm ({ dispatch }, withAddress = false) {
    ...
    dispatch('setOutgoingDirection', true)
  ...
  async getEthNetworkFee ({ commit, getters }) {
    ...
    try {
      ...
      const gasLimit = EthereumGasLimits[+getters.isOutgoingDirection][knownAsset ? getters.asset.symbol : KnownBridgeAsset.Other]
  ...
  async generateHistoryItem ({ commit, getters, dispatch }, playground) {
    await dispatch('setHistoryItem', api.bridge.generateHistoryItem({
      type: getters.isOutgoingDirection ? Operation.EthBridgeOutgoing : Operation.EthBridgeIncoming,
  ...
  async signSoraTransactionSoraToEth ({ commit, getters, rootGetters, dispatch }, { txId }) {
    ...
    if (!getters.asset || !getters.asset.address || !getters.amount || !getters.isOutgoingDirection) {
      return
    }
    const asset = await dispatch('findRegisteredAsset')
