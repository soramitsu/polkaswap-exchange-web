  ...
  <div v-loading="parentLoading" class="bridge s-flex">
    ...
    >
      <s-card class="bridge-content" border-radius="medium" shadow="never">
        ...
        <s-card :class="isOutgoingDirection ? 'bridge-item' : 'bridge-item bridge-item--extranet'" border-radius="mini" shadow="never">
          <div class="bridge-item-header">
            <div class="bridge-item-title">
              ...
              <token-logo class="bridge-item-title-icon" :tokenSymbol="isOutgoingDirection ? 'bridge-item-xor' : 'bridge-item-eth'" size="mini" />
            </div>
            <div v-if="isNetworkAConnected && isAssetSelected" class="token-balance">
              <span class="token-balance-title">{{ t('bridge.balance') }}</span>
              <span class="token-balance-value">{{ formatBalance(isOutgoingDirection) }}</span>
            </div>
          </div>
          <div class="bridge-item-content">
            ...
            <div v-if="isNetworkAConnected && isAssetSelected" class="asset">
              ...
              <s-button class="el-button--choose-token" type="tertiary" size="small" border-radius="medium" icon="chevron-down-rounded-16" icon-position="right" @click="openSelectAssetDialog">
                ...
                {{ formatAssetSymbol(assetSymbol, !isOutgoingDirection) }}
              </s-button>
            </div>
          ...
          <div v-if="isNetworkAConnected && !isOutgoingDirection" class="bridge-item-footer">
            <s-divider />
            <s-button class="el-button--change-wallet" type="link" size="mini" @click="isOutgoingDirection ? connectInternalWallet() : changeExternalWallet()">
              <span class="bridge-item-id">{{ formatAddress(isOutgoingDirection ? getWalletAddress() : ethAddress, 8) }}</span>
              <span class="change-wallet">{{ t('bridge.changeAccount') }}</span>
            </s-button>
            <span>{{ t('bridge.connected') }}</span>
          </div>
          <s-button v-else-if="!isNetworkAConnected" class="el-button--connect" type="primary" @click="isOutgoingDirection ? connectInternalWallet() : connectExternalWallet()">
        ...
        <s-card :class="!isOutgoingDirection ? 'bridge-item' : 'bridge-item bridge-item--extranet'" border-radius="mini" shadow="never">
          <div class="bridge-item-header">
            <div class="bridge-item-title">
              ...
              <token-logo class="bridge-item-title-icon" :tokenSymbol="isOutgoingDirection ? 'bridge-item-eth' : 'bridge-item-xor'" size="mini" />
            </div>
          ...
          <div v-if="isNetworkBConnected && isOutgoingDirection" class="bridge-item-footer">
            <s-divider />
            <s-button class="el-button--change-wallet" type="link" size="mini" @click="!isOutgoingDirection ? connectInternalWallet() : changeExternalWallet()">
              <span class="bridge-item-id">{{ formatAddress(isOutgoingDirection ? ethAddress : getWalletAddress(), 8) }}</span>
              <span class="change-wallet">{{ t('bridge.changeAccount') }}</span>
            </s-button>
            <span>{{ t('bridge.connected') }}</span>
          </div>
          <s-button v-else-if="!isNetworkBConnected" class="el-button--connect" type="primary" @click="!isOutgoingDirection ? connectInternalWallet() : connectExternalWallet()">
            {{ t('bridge.connectWallet') }}
          </s-button>
        </s-card>
        <s-button
        ...
        >
          ...
          <template v-else-if="isInsufficientBalance">
            {{ t('confirmBridgeTransactionDialog.insufficientBalance', { tokenSymbol : formatAssetSymbol(assetSymbol, !isOutgoingDirection) }) }}
          </template>
...
export default class Bridge extends Mixins(
  ...
  get isNetworkAConnected () {
    return this.isOutgoingDirection ? this.isSoraAccountConnected : this.isExternalAccountConnected
  }

  get isNetworkBConnected () {
    return this.isOutgoingDirection ? this.isExternalAccountConnected : this.isSoraAccountConnected
  }
  ...
  get isMaxAvailable (): boolean {
    ...
    const balance = getAssetBalance(this.asset, { internal: this.isOutgoingDirection })
    ...
    if (isXorAccountAsset(this.asset) && this.isOutgoingDirection) {
  ...
  get isInsufficientBalance (): boolean {
    ...
    return this.isNetworkAConnected && this.isRegisteredAsset && hasInsufficientBalance(this.asset, this.amount, fee, !this.isOutgoingDirection)
  }
  ...
  handleSwitchItems (): void {
    this.setOutgoingDirection(!this.isOutgoingDirection)
    if (this.isRegisteredAsset) {
      this.getNetworkFee()
      this.getEthNetworkFee()
    }
  ...
  async handleMaxValue (): Promise<void> {
    if (this.asset && this.isRegisteredAsset) {
      await this.getNetworkFee()
      const max = getMaxValue(this.asset, this.soraNetworkFee, !this.isOutgoingDirection)
      this.setAmount(max)
    }
  }
