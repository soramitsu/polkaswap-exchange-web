      ...
      <template v-if="isInitRequestCompleted">
        ...
        <s-collapse :value="activeTransactionStep" :borders="true">
          <s-collapse-item :name="transactionSteps.from">
            <template #title>
              <div class="network-info-title">
                ...
                <h3>{{ t('bridgeTransaction.networkTitle', { network: t(formatNetwork(isOutgoingDirection, true)) }) }}</h3>
                <span :class="transactionIconStatusClasses()" />
              </div>
            </template>
            <div v-if="transactionFromHash" :class="hashContainerClasses(!isOutgoingDirection)">
            ...
            <info-line
              ...
              :value="`-${amount}`"
              :asset-symbol="formatAssetSymbol(assetSymbol, !isOutgoingDirection)"
            />
            <info-line
              :label="t('bridgeTransaction.networkInfo.transactionFee')"
              :value="isOutgoingDirection ? formattedSoraNetworkFee : formattedEthNetworkFee"
              :asset-symbol="isOutgoingDirection ? KnownSymbols.XOR : EthSymbol"
            />
            ...
            <!-- <info-line :label="t('bridgeTransaction.networkInfo.total')" :value="isOutgoingDirection ? formattedSoraNetworkFee : ethereumNetworkFee" :asset-symbol="isOutgoingDirection ? KnownSymbols.XOR : EthSymbol" /> -->
            <s-button
              v-if="isTransactionStep1"
              type="primary"
              :disabled="!(isOutgoingDirection || isValidEthNetwork) || currentState === STATES.INITIAL || isInsufficientBalance || isInsufficientXorForFee || isInsufficientEthereumForFee || isTransactionFromPending"
              @click="handleSendTransactionFrom"
            >
              <template v-if="!isOutgoingDirection && !isExternalAccountConnected">{{ t('bridgeTransaction.connectWallet') }}</template>
              <template v-else-if="!(isOutgoingDirection || isValidEthNetwork)">{{ t('bridgeTransaction.changeNetwork') }}</template>
              <span v-else-if="isTransactionFromPending" v-html="t('bridgeTransaction.pending', { network: t(`bridgeTransaction.${isOutgoingDirection ? 'sora' : 'ethereum'}`) })" />
              ...
              <template v-else>{{ t('bridgeTransaction.confirm', { direction: t(`bridgeTransaction.${isOutgoingDirection ? 'sora' : 'metamask'}`) }) }}</template>
            </s-button>
          </s-collapse-item>
          <s-collapse-item :name="transactionSteps.to">
            ...
            <div v-if="isTransactionStep2 && transactionToHash" :class="hashContainerClasses(isOutgoingDirection)">
            ...
            <info-line :label="t('bridgeTransaction.networkInfo.date')" :value="transactionDate(!isOutgoingDirection ? soraTransactionDate : ethereumTransactionDate)" />
            <info-line
              :label="t('bridgeTransaction.networkInfo.amount')"
              :value="`${amount}`"
              :asset-symbol="formatAssetSymbol(assetSymbol, isOutgoingDirection)"
            ...
            <s-button
              ...
              type="primary"
              :disabled="(isOutgoingDirection && !isValidEthNetwork) || isInsufficientXorForFee || isInsufficientEthereumForFee || isTransactionToPending"
              @click="handleSendTransactionTo"
            >
              <template v-if="isOutgoingDirection && !isExternalAccountConnected">{{ t('bridgeTransaction.connectWallet') }}</template>
              <template v-else-if="isOutgoingDirection && !isValidEthNetwork">{{ t('bridgeTransaction.changeNetwork') }}</template>
              <span v-else-if="isTransactionToPending" v-html="t('bridgeTransaction.pending', { network: t(`bridgeTransaction.${!isOutgoingDirection ? 'sora' : 'ethereum'}`) })" />
...
export default class BridgeTransaction extends Mixins(WalletConnectMixin, LoadingMixin, NetworkFormatterMixin, NumberFormatterMixin) {
  ...
  @Getter('isOutgoingDirection', { namespace }) isOutgoingDirection!: boolean
  ...
  get isTransactionFromPending (): boolean {
    return this.currentState === (this.isOutgoingDirection ? STATES.SORA_PENDING : STATES.ETHEREUM_PENDING)
  }
  ...
  get isTransactionFromFailed (): boolean {
    return this.currentState === (this.isOutgoingDirection ? STATES.SORA_REJECTED : STATES.ETHEREUM_REJECTED)
  }
  ...
  get isTransferCompleted (): boolean {
    const isTransactionCompleted = this.currentState === (!this.isOutgoingDirection ? STATES.SORA_COMMITED : STATES.ETHEREUM_COMMITED)
  ...
  get transactionDetails (): string {
    ...
    return this.t('bridgeTransaction.details', {
      from: `${this.amount} ${formatAssetSymbol(this.assetSymbol, !this.isOutgoingDirection)}`,
      to: `${this.amount} ${formatAssetSymbol(this.assetSymbol, this.isOutgoingDirection)}`
    })
  }
  ...
  get statusFrom (): string {
    ...
    if (this.isTransactionStep1) {
      if (this.currentState === (this.isOutgoingDirection ? STATES.SORA_PENDING : STATES.ETHEREUM_PENDING)) {
        return this.t('bridgeTransaction.statuses.pending') + '...'
      }
      if (this.currentState === (this.isOutgoingDirection ? STATES.SORA_REJECTED : STATES.ETHEREUM_REJECTED)) {
  ...
  get transactionFromDate (): string {
    return this.transactionDate(this.isOutgoingDirection ? this.soraTransactionDate : this.ethereumTransactionDate)
  }
  ...
  get isInsufficientBalance (): boolean {
    ...
    const fee = this.isOutgoingDirection ? this.soraNetworkFee : this.ethereumNetworkFee

    return hasInsufficientBalance(this.asset, this.amount, fee, !this.isOutgoingDirection)
  }
  ...
  handleOpenEtherscan (): void {
    const hash = this.isOutgoingDirection ? this.transactionToHash : this.transactionFromHash
  ...
  async initializeTransactionStateMachine () {
    ...
    const machineStates = this.isOutgoingDirection ? SORA_ETHEREUM_STATES : ETHEREUM_SORA_STATES
  ...
  failedClass (isSecondTransaction: boolean): string {
    if (!isSecondTransaction) {
      return this.currentState === (this.isOutgoingDirection ? STATES.SORA_REJECTED : STATES.ETHEREUM_REJECTED) ? 'info-line--error' : ''
    }
    return this.currentState === (!this.isOutgoingDirection ? STATES.SORA_REJECTED : STATES.ETHEREUM_REJECTED) ? 'info-line--error' : ''
  }
